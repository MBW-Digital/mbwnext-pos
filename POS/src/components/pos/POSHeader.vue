		...
		<div class="flex py-2 sm:py-3">
			...
			<div class="w-16 flex-shrink-0 flex items-center justify-center">
				<button
					...
					:aria-label="'MBW Next POS'"
					:title="__('MBW Next POS')"
				>
				...
				<div class="flex items-center gap-1 sm:gap-4 min-w-0 flex-1 overflow-hidden">
					<div class="min-w-0 flex-shrink overflow-hidden">
						<div class="flex items-center gap-1 sm:gap-2">
							<h1 class="text-xs sm:text-base font-bold text-gray-900 truncate flex-shrink">{{ 'MBW Next POS' }}</h1>
						</div>
					...
					<div class="hidden lg:flex items-center gap-4 ms-6 flex-shrink-0">
						...
						<!-- HR work shift (Roster / Shift Assignment) -->
						<StatusBadge
							v-if="hasOpenShift && hrScheduleBadge?.value"
							:variant="hrScheduleBadge.variant"
							size="xs"
							:icon="calendarShiftIcon"
							:label="hrScheduleBadge.label"
							:value="hrScheduleBadge.value"
							class="max-w-[min(22rem,calc(100vw-400px))] [&_span:last-child]:truncate"
							:title="hrScheduleBadge.value"
						/>
					</div>
...
const props = defineProps({
	...
	/** HR roster line for header: { label, value, variant: 'green'|'orange'|'gray' } */
	hrScheduleBadge: {
		type: Object,
		default: null,
	},
